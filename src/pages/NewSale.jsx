...
const NewSale = () => {
  ...
  // 1. ESTADO PARA EL USUARIO LOGUEADO
  const [usuarioActual, setUsuarioActual] = useState(null);

  useEffect(() => {
    const inicializarPantalla = async () => {
      try {
        // 2. LEEMOS EL USUARIO DEL LOCALSTORAGE (Tu Login debe guardarlo aquí)
        const usuarioGuardado = localStorage.getItem('usuario');
        
        // Si no hay nadie en el storage, ponemos uno de prueba temporal
        const usuario = usuarioGuardado 
          ? JSON.parse(usuarioGuardado) 
          : { idUsuario: 1, nombreCompleto: 'Usuario Temporal', rol: { nombre: 'CAJERO' } };
        
        setUsuarioActual(usuario);

        // 3. LÓGICA CONDICIONAL POR ROLES
        if (usuario.rol?.nombre === 'ADMIN') {
          // El Admin no necesita abrir caja obligatoriamente
          setCajaBloqueada(false);
        } else {
          // El Cajero sí debe ser verificado en la Base de Datos
  ...
  // Enfocar el lector cuando la caja se desbloquee
  useEffect(() => {
    if (!cajaBloqueada && lectorRef.current) {
  ...
  const handleAbrirCaja = async (e) => {
    ...
    try {
      const nuevoTurno = {
        idUsuario: usuarioActual.idUsuario, // <-- Usa el ID real
        nombreCajero: usuarioActual.nombreCompleto, // <-- Usa el nombre real
  ...
  const handleCerrarCaja = async (e) => {
    e.preventDefault();
    if (montoCierreDeclarado === '' || Number(montoCierreDeclarado) < 0) {
      return toast.error("Ingresa el monto físico real de tu cajón");
    }

    try {
      const datosCierre = {
        montoFinal: Number(montoCierreDeclarado),
        totalVentas: turnoActivo.totalVentas || 0
      };

      await cerrarCaja(turnoActivo.idTurno, datosCierre);
      ...
      toast.error("Hubo un error al cerrar la caja.");
    }
  };
  ...
  const handleProcesarVenta = async () => {
    ...
    if (totalIngresado === 0) finalEfectivo = total;
    else if (Math.abs(saldoRestante) > 0.01 && saldoRestante > 0) return toast.error(`Falta cubrir S/ ${saldoRestante.toFixed(2)}`);
    ...
    const datosVenta = {
      ...
      idTurno: turnoActivo?.idTurno || null, // Se envía el turno solo si existe
    ...
    try {
      ...
      if (turnoActivo) {
        setTurnoActivo(prev => ({ ...prev, totalVentas: (prev.totalVentas || 0) + total }));
      }
      ...
      setCarrito([]); setDocumentoCliente(''); setPagoEfectivo(''); setPagoYape(''); setPagoPlin(''); setPagoTarjeta('');
        ...
        <div className="fixed inset-0 bg-slate-900/90 z-50 flex flex-col items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md text-center transform transition-all">
            ...
            <form onSubmit={handleAbrirCaja}>
              ...
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl text-lg shadow-lg border-none cursor-pointer flex justify-center items-center gap-2">
        ...
        <div className="flex-1 flex p-6 gap-6 h-[calc(100vh-80px)] overflow-hidden">
          ...
          <div className="w-[420px] flex flex-col gap-4 overflow-y-auto pr-1">
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border border-slate-200 mb-3"><span className="text-xs font-semibold text-slate-700">Enviar a SUNAT</span><button type="button" onClick={() => setSunatActivo(!sunatActivo)} className="bg-transparent border-none cursor-pointer">{sunatActivo ? <ToggleRight size={38} className="text-green-500" /> : <ToggleLeft size={38} className="text-slate-400" />}</button></div>
              {sunatActivo && (
            ...
            <div className="bg-slate-900 text-white rounded-xl shadow-xl p-5">
              ...
              <button onClick={handleProcesarVenta} disabled={procesando || cajaBloqueada} className="w-full text-white py-3.5 rounded-xl font-bold border-none cursor-pointer bg-blue-600 hover:bg-blue-500 disabled:bg-slate-600">
                {procesando ? 'Procesando...' : (sunatActivo ? 'Emitir Comprobante' : 'Cobrar e Imprimir')}
              </button>
            </div>
          </div>
        </div>
