    ...
    return ventasFiltradas.map((v) => {
      ...
      return (
        <tr key={v.idVenta} className="hover:bg-slate-50/80 transition-colors">
          <td className="px-5 py-3.5"><p className="font-bold text-slate-700">#00{v.idVenta}</p><span className="text-[10px] text-slate-400">{new Date(v.fechaVenta).toLocaleString()}</span></td>
          ...
          <div className="flex-1 overflow-auto">
            <table className="w-full text-left border-collapse min-w-[600px]">
              <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-bold tracking-wider border-b border-slate-200 sticky top-0 z-10">
                <tr><th className="px-5 py-3">ID / Fecha</th><th className="px-5 py-3">Tipo</th><th className="px-5 py-3">Desglose de Pago</th><th className="px-5 py-3 text-right">Total</th><th className="px-5 py-3 text-center">Acción</th></tr>
              </thead>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-lg shadow-2xl modal-enter overflow-hidden flex flex-col max-h-[90vh]">
            <div className="bg-slate-900 text-white p-4 flex justify-between items-center">
              <h3 className="font-bold flex items-center gap-2"><ShoppingCart size={18}/> Detalle de Venta #00{ventaSeleccionada.idVenta}</h3>
